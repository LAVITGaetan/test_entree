...
<body>
    ...
    <form class="formulaire_ajout" method="post">
        ...
        <label class=" ajout_label">Heure début
            <input class="formulaire_entree" type="number" name="heure_debut">
            <input class="formulaire_entree" type="number" name="minute_debut">
        </label>

        <!-- Selection de l'heure de fin  -->
        <label class="ajout_label">Heure fin
            <input class="formulaire_entree" type="number" name="heure_fin">
            <input class="formulaire_entree" type="number" name="minute_fin">
        </label>


        <!-- Bouton de validation du formulaire -->
        <input class="formulaire_bouton" type="submit" value="Attribuer" name="attribuer">
    ...
    <div class="poste_container">
        ...
        reservation.date, reservation.heure_debut, reservation.minute_debut, reservation.heure_fin, reservation.minute_fin, reservation.id AS id_reservation FROM poste,utilisateur,reservation WHERE reservation.id_utilisateur = utilisateur.id
        ...
        //Attribuer un poste
        if (isset($_POST['attribuer']) && !empty($_POST['date']) && !empty($_POST['heure_debut']) && !empty($_POST['minute_debut']) && !empty($_POST['heure_fin']) && !empty($_POST['minute_fin'])) {
            $requete = 'INSERT INTO reservation VALUES(NULL, "' . $_POST['id_poste'] . '", "' . $_POST['id_utilisateur'] . '", "' . $_POST['date'] . '", "' . $_POST['heure_debut'] . '", "' . $_POST['minute_debut'] . '", "' . $_POST['heure_fin'] . '", "' . $_POST['minute_fin'] . '")';
            $reponse = $bdd->query($requete);
